...
class LayersDatabase {
	...
	public function getLayerSet( int $layerSetId ): ?array {
		...
		if ( !$row ) {
			// Don't cache null results — isset() can't detect them, so they
			// waste cache slots and evict valid entries (LOW-v24-7)
			return null;
		}
	 ...
	 * @return ?array Layer set data or null if not found
	 */
	public function getLatestLayerSet( string $imgName, string $sha1, ?string $setName = null, int $page = 1 ): ?array {
