...
class WikitextHooks
{
    ...
    public static function onImageBeforeProduceHTML(
        ...
        try {
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                ...
                $logger->info('Layers: ImageBeforeProduceHTML fired for file: ' . $fileName . ', hasLinkAttribs: ' . ($hasLinkAttribs ? 'yes' : 'no'));
            }
            ...
            if (isset($linkAttribs['href'])) {
                ...
                if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                    $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                    $logger->info('Layers: ImageBeforeProduceHTML detected layersFlag: ' . ($layersFlag ?: 'none') . ' from href: ' . substr($href, 0, 200));
                }
            } else {
                if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                    $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
            ...
            if (( $layersFlag === 'all' || $layersFlag === 'on' || $layersFlag === true ) && $file) {
                if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                    $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                    $logger->info('Layers: ImageBeforeProduceHTML attempting injection for file: ' . $file->getName());
                }
                ...
                if ($latest && isset($latest['data'])) {
                    ...
                    if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                        $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                        $logger->info('Layers: ImageBeforeProduceHTML no layer data found in database for: ' . $file->getName());
                    }
                }
            } else {
                if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                    $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                    $hasFile = $file ? 'yes' : 'no';
                    $logger->info('Layers: ImageBeforeProduceHTML skipping injection - layersFlag: ' . ($layersFlag ?: 'none') . ', hasFile: ' . $hasFile);
                }
            }
        } catch (\Throwable $e) {
    ...
    public static function onMakeImageLink2(
        ...
        try {
            ...
            if (
                $file && ( $layersFlag === 'all' || $layersFlag === 'on' ||
                ...
                if ($layersArray !== null) {
                    ...
                    $res = preg_replace_callback('/<img\b([^>]*)>/i', static function ($m) {
                        $attrs = $m[1];
                        if (preg_match('/\bdata-layers-intent\s*=\s*("|\')(.*?)(\1)/i', $attrs)) {
                            $attrs = preg_replace('/\bdata-layers-intent\s*=\s*("|\')(.*?)(\1)/i', 'data-layers-intent="on"', $attrs);
                        } else {
            ...
            if ($file) {
                ...
                if ($param === null && is_array($attribs)) {
                    foreach ([ 'href', 'data-mw-href' ] as $k) {
                        if (isset($attribs[$k]) && preg_match('/(?:[\?&#]|\|)layers=([^\s"\'<>|&#]+)/i', (string)$attribs[$k], $mm)) {
                            $param = $mm[1];
                            break;
                        }
                    }
                ...
                if ($param === null) {
                    return true; // No explicit layers parameter detected
                }
                ...
                if ($param === 'on' || $param === 'all') {
                    $latest = $db->getLatestLayerSet($file->getName(), $file->getSha1());
                    if ($latest && isset($latest['data'])) {
                        $layersArray = isset($latest['data']['layers']) && is_array($latest['data']['layers']) ? $latest['data']['layers'] : $latest['data'];
                    }
                } elseif (preg_match('/^id:(\d+)$/', $param, $idM)) {
                    $ls = $db->getLayerSet((int)$idM[1]);
                    if ($ls && isset($ls['data'])) {
                        $layersArray = isset($ls['data']['layers']) && is_array($ls['data']['layers']) ? $ls['data']['layers'] : $ls['data'];
                    }
    ...
    public static function onGetLinkParamDefinitions(array &$params): bool
    {
        ...
        if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
            $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
            $logger->info('Layers: onGetLinkParamDefinitions called, registering layers parameter');
        }
    ...
    public static function onParserGetImageLinkParams(array &$params): bool
    {
        ...
        if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
            ...
            $logger->info("Layers: onParserGetImageLinkParams called, current params: $currentParams");
        }
    ...
    public static function onParserGetImageLinkOptions(array &$options): bool
    {
        ...
        if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
            ...
            $logger->info("Layers: onParserGetImageLinkOptions called, current options: $currentOptions");
        }
    ...
    // If transform params provided data, render regardless of missing flag (treat as on)
        if ($layerData !== null) {
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                $logger->info(
                    'Layers: Added layer data attribute with ' . count($layerData) . ' layers, instance: ' . $instanceId
                );
            }
        } else {
            // Add unique instance marker to prevent cross-instance interference
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                $logger->info('Layers: No layer data to add, instance: ' . $instanceId);
            }
    ...
    public static function onParserMakeImageParams($title, $file, array &$params, $parser): bool
    {
        ...
        if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
            ...
            $logger->info("Debug logging for onParserMakeImageParams - fileName: $fileName, titleText: $titleText, hasLayersParam: " . ($hasLayersParam ? 'yes' : 'no') . ", layersValue: $layersValue, hasLayerParam: " . ($hasLayerParam ? 'yes' : 'no') . ", layerValue: $layerValue, paramsKeys: " . implode(',', $paramsKeys));
        }
        ...
        if (!isset($params['layers'])) {
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                $logger->info("Debug logging for onParserMakeImageParams - No layers parameter found, returning early");
            }
            return true;
        }
        ...
        if ($layersRaw === true || $layersRaw === 'on' || $layersRaw === 'all') {
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                $logger->info("Debug logging for onParserMakeImageParams - Processing layers='all' or 'on', layersRaw: $layersRaw, file exists: " . ($file ? 'yes' : 'no') . ", pageHasLayers set to true");
            }
            if ($file) {
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                ...
                $logger->info("Debug logging for onParserMakeImageParams - After processing layers='all': layersNormalized: $layersNormalized, hasLayerData: " . ($hasLayerData ? 'yes' : 'no') . ", hasLayerSetId: " . ($hasLayerSetId ? 'yes' : 'no'));
            }
            return true;
        }
        ...
        try {
            ...
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $text, $matches)) {
                    ...
                    if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                        $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                        $logger->info('Layers: ParserBeforeInternalParse detected layers parameter: ' . $matches[0] . ', setting pageHasLayers=true');
                    }
                    break;
                }
            }
            ...
            if (!self::$pageHasLayers && (strpos($text, 'layers=') !== false || strpos($text, 'layer=') !== false)) {
                if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                    $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                    $logger->info('Layers: Found layers= or layer= text in wikitext, setting pageHasLayers=true as fallback');
                }
                self::$pageHasLayers = true;
            }
            ...
            if (\class_exists('\\MediaWiki\\Logger\\LoggerFactory')) {
                $logger = \call_user_func([ '\\MediaWiki\\Logger\\LoggerFactory', 'getInstance' ], 'Layers');
                $logger->error('Layers: Error in ParserBeforeInternalParse: ' . $e->getMessage());
            }
        }
