		...
		if ( $this->paramExtractor->isEditorLink( $linkType ) ) {
			...
			if ( $this->paramExtractor->isEditorModal( $linkType ) ) {
				...
				if ( $this->paramExtractor->isEditorNewtab( $linkType ) ) {
					...
					$linkAttribs['rel'] = 'noopener';
					$linkAttribs['data-layers-link'] = 'editor-newtab';
					$attribs['data-layers-link'] = 'editor-newtab';
					$linkAttribs['title'] = wfMessage( 'layers-link-editor-newtab-title' )->text();
					$this->log( "Applied newtab editor link for: $filename" );
				} else {
					$this->log( "Applied editor link for: $filename" );
				}
			}
		} elseif ( $this->paramExtractor->isViewerLink( $linkType ) ) {
			// Viewer/lightbox mode - use full size image with layers overlay
			// Get full-size image URL
			$fullUrl = $file->getFullUrl();

			$linkAttribs['href'] = $fullUrl;
			...
			$this->log( "Applied viewer link for: $filename" );
		}
	}
}
