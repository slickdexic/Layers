...
| R4.04 no document model | **Mostly fixed.** Navigation offers **Save / Discard / Cancel**, and the page navigator marks annotated and dirty pages and lists every annotated page in its tooltip (new `pagesWithLayers` API field). v1.5.94 removed the reload as well, so a page turn keeps the editor alive. The remaining gap is the save model — one page dirty at a time — carried as R4.60 stage 2. |
