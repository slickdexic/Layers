...
| R4.04 no document model | **Fixed (v1.5.95).** Navigation is in place and silent: unsaved work for pages the reader has left is held in `PageBuffer`, one **Save** writes every changed page, and the exit and tab-close guards are document-level so pages edited earlier cannot be discarded without a prompt that names them. The page navigator marks annotated and dirty pages and lists both annotated and unsaved pages in its tooltip. Remaining gap is R4.63 — the annotated list counts pages whose sets are empty. |
