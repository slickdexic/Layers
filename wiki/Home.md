...
- **RichText font-size parity** — REL1_43 now allows inline rich-text runs up to 1,000px, matching the layer-level textbox limit.
- **Help dialog reliability** — Removed stale debug-only DOM probes so the help modal always reaches Escape-key handling and logs failures through `mw.log.error()`.
- **Test Suite** — 11,494 tests passing (168 suites), 92.88% statement coverage
