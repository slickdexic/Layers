...
( function () {
	...
	exportAsSVG( options = {} ) {
		return new Promise( ( resolve, reject ) => {
			try {
				...
				// Get SVGExporter class
				const SVGExporter = window.Layers && window.Layers.Export && window.Layers.Export.SVGExporter;
				if ( !SVGExporter ) {
					reject( new Error( 'SVG exporter not loaded' ) );
					return;
				}

				// Get dimensions
				const width = this.editor.stateManager.get( 'baseWidth' ) || canvasManager.canvas.width;
				const height = this.editor.stateManager.get( 'baseHeight' ) || canvasManager.canvas.height;
				...
				const exporter = new SVGExporter( {
					...
					backgroundImage: canvasManager.backgroundImage,
				...
				// Export to blob
				const blob = exporter.exportToBlob( layers );
				resolve( blob );
	...
	/**
	 * Export and download layers as SVG file.
	 *
	 * @param {Object} options - Export options
	 * @param {string} options.filename - Custom filename (optional)
	 * @param {boolean} options.includeBackground - Include background image (default: true)
	...
	downloadAsSVG( options = {} ) {
		...
		const baseName = filename
			.replace( /^File:/i, '' )
			.replace( /\.[^/.]+$/, '' );
		// Format: ImageName-LayerSetName.svg (omit -default for default set)
		const setNamePart = currentSetName === 'default' ? '' : `-${ currentSetName }`;
		...
		if ( options.filename ) {
			...
			const hasExt = /\.svg$/i.test( sanitized );
			downloadName = hasExt ? sanitized : sanitized + '.svg';
		} else {
			downloadName = this.sanitizeFilename( `${ baseName }${ setNamePart }` ) + '.svg';
		}
		...
		this.exportAsSVG( options ).then( ( blob ) => {
			...
			const msg = this.getMessage( 'layers-export-svg-success', 'SVG exported successfully' );
			mw.notify( msg, { type: 'success' } );
		} ).catch( ( error ) => {
			this.editor.uiManager.hideSpinner();
			if ( typeof mw !== 'undefined' && mw.log ) {
				mw.log.error( '[APIManager] SVG export failed:', error );
			}
			const msg = this.getMessage( 'layers-export-svg-failed', 'Failed to export SVG' );
			mw.notify( msg, { type: 'error' } );
		} );
	}
