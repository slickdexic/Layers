...
( function () {
	...
	/**
	 * CanvasUtilities - Collection of pure utility functions
	 * @class
	 */
	function CanvasUtilities() {
		// Static utility class - no instance properties
	}
	 ...
	 */
	CanvasUtilities.sanitizeNumber = function ( value, defaultVal, min, max ) {
	 ...
	 */
	CanvasUtilities.sanitizeColor = function ( color, defaultColor ) {
	 ...
	 */
	CanvasUtilities.sanitizeFontFamily = function ( fontFamily ) {
		...
		for ( let i = 0; i < safeFonts.length; i++ ) {
			if ( fontFamily.toLowerCase().includes( safeFonts[ i ].toLowerCase() ) ) {
				return fontFamily;
			}
		}
	 ...
	 */
	CanvasUtilities.rectanglesIntersect = function ( rect1, rect2 ) {
	 ...
	 */
	CanvasUtilities.clamp = function ( value, min, max ) {
	 ...
	 */
	CanvasUtilities.getCursorForHandle = function ( handle ) {
	 ...
	 * @return {string} CSS cursor value
	 */
	CanvasUtilities.getToolCursor = function ( tool ) {
		switch ( tool ) {
			case 'pointer':
				return 'default';
			case 'pan':
	 ...
	 */
	CanvasUtilities.isValidBlendMode = function ( blendMode ) {
		const validModes = [
		...
		return validModes.indexOf( blendMode ) !== -1;
	};
	 ...
	 */
	CanvasUtilities.calculateAspectFit = function ( sourceWidth, sourceHeight, targetWidth, targetHeight ) {
	 ...
	 */
	CanvasUtilities.deepClone = function ( obj ) {
