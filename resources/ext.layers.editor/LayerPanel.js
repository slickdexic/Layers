...
( function () {
    ...
    LayerPanel.prototype.createInterface = function () {
        ...
        this.addButton.className = 'layers-add-button';
        this.addButton.textContent = mw.msg( 'layers-add-layer' );
    ...
    LayerPanel.prototype.getDefaultLayerName = function ( layer ) {
        switch ( layer.type ) {
            case 'text': return 'Text: ' + ( layer.text || 'Empty' ).substring( 0, 20 );
    ...
    LayerPanel.prototype.showAddLayerMenu = function () {
        // Simple implementation - could be enhanced with a proper menu
        var tools = [
            { id: 'text', name: mw.msg( 'layers-tool-text' ) },
            { id: 'rectangle', name: mw.msg( 'layers-tool-rectangle' ) },
            { id: 'circle', name: mw.msg( 'layers-tool-circle' ) },
            { id: 'arrow', name: mw.msg( 'layers-tool-arrow' ) }
        ];
        
        var menu = tools.map( function ( tool ) {
            return tool.name;
        } ).join( '\n' );
        
        var choice = prompt( 'Select tool to add:\n' + menu );
        if ( choice ) {
            var selectedTool = tools.find( function ( tool ) {
                return tool.name.toLowerCase() === choice.toLowerCase();
            } );
            
            if ( selectedTool ) {
                this.editor.setCurrentTool( selectedTool.id );
            }
        }
    };
    ...
    LayerPanel.prototype.reorderLayers = function ( draggedId, targetId ) {
        var draggedIndex = this.layers.findIndex( function ( layer ) {
            return layer.id === draggedId;
        } );
        
        var targetIndex = this.layers.findIndex( function ( layer ) {
