...
( function () {
	...
	class LayersViewer {
		...
		drawBackgroundOnCanvas() {
			...
			if ( isHidden ) {
				// Clear to transparent so blend modes work with page background
				this.ctx.clearRect( 0, 0, this.canvas.width, this.canvas.height );
				return;
			}
		...
		scaleLayerCoordinates( layer, sx, sy, scaleAvg ) {
			...
			// Callout tail tip position - must be scaled for proper tail rendering
			if ( typeof L.tailTipX === 'number' ) {
				L.tailTipX = L.tailTipX * sx;
			}
