	...
	class ShapeRenderer {
		...
		drawTextBox( layer, options ) {
			...
			if ( layer.text && layer.text.length > 0 ) {
				...
				// Split text into lines and wrap (pass fontWeight and fontStyle)
				...
				// Text stroke properties (stroke is drawn first, then fill on top)
				...
				for ( let i = 0; i < lines.length; i++ ) {
					...
					if ( currentY + fontSize <= y + height ) {
						...
						if ( hasTextShadow ) {
							...
							this.ctx.shadowBlur = textShadowBlur;
							this.ctx.shadowOffsetX = textShadowOffsetX;
							this.ctx.shadowOffsetY = textShadowOffsetY;
						}
						...
						if ( hasTextStroke ) {
							...
							this.ctx.strokeText( lines[ i ], textX, currentY );
						...
						// Draw text fill
						this.ctx.fillText( lines[ i ], textX, currentY );
