...
( function () {
	...
	// Get TailCalculator for geometric calculations
	const TailCalculator = ( typeof window !== 'undefined' && window.Layers &&
		window.Layers.TailCalculator ) || ( typeof window !== 'undefined' && window.TailCalculator );
	...
	class CalloutRenderer {
		...
		constructor( ctx, config ) {
			...
			// Initialize TailCalculator for geometric calculations
			this.tailCalculator = TailCalculator ? new TailCalculator() : null;
		}
		...
		// ========================================================================
		// Callout Path Drawing (delegates to TailCalculator for geometry)
		// ========================================================================

		/**
		 * Calculate the closest point on the rectangle perimeter to a given point.
		 * Delegates to TailCalculator.
		 *
		 * @private
		 ...
		 * @return {Object} Object with edge, baseX, baseY, tangentX, tangentY
		 */
		getClosestPerimeterPoint( px, py, x, y, width, height, cornerRadius ) {
			if ( this.tailCalculator ) {
				return this.tailCalculator.getClosestPerimeterPoint(
					px, py, x, y, width, height, cornerRadius
				);
			}
			// Fallback if TailCalculator not available
		 ...
		 * @param {number} width - Rectangle width
		 * @param {number} height - Rectangle height
		 * @param {number} tipX - Tail tip X position
		 * @param {number} tipY - Tail tip Y position
		 * @param {number} cornerRadius - Corner radius to avoid
		 * @return {Object} Object with base1, base2, tip, edge
		 */
		getTailFromTipPosition( x, y, width, height, tipX, tipY, cornerRadius ) {
			if ( this.tailCalculator ) {
				return this.tailCalculator.getTailFromTipPosition(
					x, y, width, height, tipX, tipY, cornerRadius
				);
			}
			// Fallback if TailCalculator not available
			const centerX = x + width / 2;
			return {
				base1: { x: centerX - 10, y: y + height },
				base2: { x: centerX + 10, y: y + height },
				tip: { x: tipX, y: tipY },
				edge: 'bottom'
			};
		}
		...
		/**
		 * Get the straight edge before a corner. Delegates to TailCalculator.
		 *
		 * @private
		...
		_getEdgeBeforeCorner( corner ) {
			if ( this.tailCalculator ) {
				return this.tailCalculator._getEdgeBeforeCorner( corner );
			}
			const mapping = { tr: 'top', br: 'right', bl: 'bottom', tl: 'left' };
			return mapping[ corner ] || 'top';
		}

		/**
		 * Get the straight edge after a corner. Delegates to TailCalculator.
		 *
		 * @private
		 * @param {string} corner - Corner identifier
		...
		_getEdgeAfterCorner( corner ) {
			if ( this.tailCalculator ) {
				return this.tailCalculator._getEdgeAfterCorner( corner );
			}
			const mapping = { tr: 'right', br: 'bottom', bl: 'left', tl: 'top' };
		 ...
		 * @return {Object} Object with base1, base2, tip
		 */
		getTailCoordinates( x, y, width, height, direction, position, tailSize, cornerRadius ) {
			if ( this.tailCalculator ) {
				return this.tailCalculator.getTailCoordinates(
					x, y, width, height, direction, position, tailSize, cornerRadius
				);
			}
			// Fallback if TailCalculator not available
			const centerX = x + width / 2;
			return {
				base1: { x: centerX - 10, y: y + height },
				base2: { x: centerX + 10, y: y + height },
				tip: { x: centerX, y: y + height + tailSize }
			};
		}
